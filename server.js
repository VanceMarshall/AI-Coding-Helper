...
const MODEL_OPTIONS_PATH = path.join(__dirname, "config", "model_options.json");
...
async function loadModelOptions() {
  try {
    const text = await fs.readFile(MODEL_OPTIONS_PATH, "utf8");
    const parsed = JSON.parse(text);
    if (!parsed || typeof parsed !== "object") throw new Error("invalid");
    if (!Array.isArray(parsed.options)) parsed.options = [];
    if (!parsed.lastSyncedAt) parsed.lastSyncedAt = null;
    return parsed;
  } catch (err) {
    if (err.code === "ENOENT") return { lastSyncedAt: null, options: [] };
    throw err;
  }
}

async function saveModelOptions(payload) {
  await fs.writeFile(MODEL_OPTIONS_PATH, JSON.stringify(payload, null, 2), "utf8");
}
...
// (Older frontend builds call /api/projects, while some server versions used /api/repos.)
// Keeping both prevents the project dropdown from breaking on redeploys.
// -----------------------------------------------------------------------------
...
app.get("/api/projects/:owner/:repo/files", async (req, res) => {
  try {
    ...
    const files = await listRepoFiles(fullName);
    res.json({ files });
  } catch (err) {
...
// API-style admin routes used by the frontend (kept separate from /admin/* to avoid breaking older deployments)
app.post("/api/admin/login", async (req, res) => {
  // Delegate to the same logic as /admin/login
  try {
    const { password } = req.body;
    if (!password) return res.status(400).json({ error: "Password required" });
...
app.get("/api/admin/auth-status", async (req, res) => {
  try {
    const secrets = await loadSecrets();
    const hasPassword = !!secrets.adminPassword;
    const sessionToken = req.headers["x-admin-session"];
    const session = sessionToken ? adminSessions.get(sessionToken) : null;
    const authenticated = !hasPassword || (session && session.expires > Date.now());
    if (sessionToken && session && session.expires <= Date.now()) adminSessions.delete(sessionToken);
    res.json({ ok: true, hasPassword, authenticated });
  } catch (err) {
...
// Read & write provider API keys (stored in config/secrets.json, with Railway env vars taking precedence)
app.get("/api/admin/api-keys", requireAdminAuth, async (req, res) => {
  try {
    ...
    const apiKeys = await Promise.all(providers.map(async ({ provider, name }) => {
      const { key, source } = await getApiKeyWithFallback(provider);
      return { provider, name, maskedKey: maskKey(key), source, connected: !!key };
    }));
...
app.delete("/api/admin/api-keys/:provider", requireAdminAuth, async (req, res) => {
  try {
    ...
    const secrets = await loadSecrets();
    if (secrets.apiKeys) delete secrets.apiKeys[provider];
    await saveSecrets(secrets);
    await reloadProvidersWithSecrets();
    res.json({ ok: true });
  } catch (err) {
...
// Update a specific model config (used by admin panel)
app.post("/api/config/models", requireAdminAuth, async (req, res) => {
  try {
    const { modelKey, updates } = req.body || {};
    if (!modelKey || !updates) return res.status(400).json({ error: "modelKey and updates are required" });
    const config = await loadConfig();
    if (!config.models?.[modelKey]) return res.status(404).json({ error: `Unknown modelKey: ${modelKey}` });
...
// Model options for dropdowns (curated + sync)
app.get("/api/admin/model-options", requireAdminAuth, async (req, res) => {
  try {
    const payload = await loadModelOptions();
...
app.post("/api/admin/model-options/sync", requireAdminAuth, async (req, res) => {
  try {
    const payload = await loadModelOptions();
    const existing = new Map(payload.options.map(o => [`${o.provider}:${o.model}`, o]));

    // Cost map (USD per 1M tokens) - pricing sources documented in README/admin UI.
    const COSTS = {
      "google:gemini-3-flash-preview": { inputCost: 0.50, outputCost: 3.00 },
      "openai:gpt-5.2": { inputCost: 1.75, outputCost: 14.00 },
      "anthropic:claude-sonnet-4-5": { inputCost: 3.00, outputCost: 15.00 },
      "anthropic:claude-opus-4-5": { inputCost: 5.00, outputCost: 25.00 },
    };

    // Google Gemini list-models (best-effort)
    const { key: googleKey } = await getApiKeyWithFallback("google");
    if (googleKey) {
      try {
        const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(googleKey)}`);
        if (r.ok) {
        ...
        console.warn("Gemini model sync failed (continuing):", err.message);
      }
    }

    // OpenAI list-models (best-effort)
    const { key: openaiKey } = await getApiKeyWithFallback("openai");
    if (openaiKey) {
      try {
        ...
        console.warn("OpenAI model sync failed (continuing):", err.message);
      }
    }

    payload.options = Array.from(existing.values());
    payload.lastSyncedAt = new Date().toISOString();
    await saveModelOptions(payload);
...
// Frontend compatibility: /api/projects is the canonical path used by the UI
app.get("/api/projects", async (req, res) => {
  try {
    const repos = await listRepos();
    res.json(repos);
...
// Frontend compatibility: /api/projects/:owner/:repo/files
app.get("/api/projects/:owner/:repo/files", async (req, res) => {
  try {
    const { owner, repo } = req.params;
    const files = await listRepoFiles(`${owner}/${repo}`);
...
// Lightweight stats endpoint used by the admin UI (public)
